...
4. **Configure terminal options** — If your changes affect terminal output or display, open \`/config\` and adjust relevant terminal settings (progress bar, notifications, turn duration, etc.) to match the expected behavior.
5. **Commit and push** — Commit all changes with a clear message, push the branch, and create a PR with \`gh pr create\`. Use a descriptive title. If \`gh\` is not available or the push fails, note it in your final message.
